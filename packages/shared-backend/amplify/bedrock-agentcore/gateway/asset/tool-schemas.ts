...
export const searchAssetKnowledgeBaseToolSchema = ToolSchema.fromInline([
  {
    name: "search-asset-knowledge-base",
    description:
...
export const deleteAssetToolSchema = ToolSchema.fromInline([
  {
    name: "delete-asset",
    description: "資産を物理削除します（復元不可）。ユーザーが削除を確認した後に使用します。",
    inputSchema: {
      type: SchemaDefinitionType.OBJECT,
      properties: {
        id: {
...
export const listAssetsToolSchema = ToolSchema.fromInline([
  {
    name: "list-assets",
    description:
      "GSIで資産を取得します。利用可能な組み合わせは `departmentId` 単独/`departmentId+status`、`assigneeSub` 単独/`assigneeSub+status`、`assetTypeId` 単独のみです。これ以外の条件は呼び出さず search-asset-knowledge-base を使用してください。",
    inputSchema: {
      type: SchemaDefinitionType.OBJECT,
      properties: {
        departmentId: {
        ...
        status: {
          type: SchemaDefinitionType.STRING,
          description: "inStock|lent|inRepair|disposed。departmentId または assigneeSub と併用時のみ使用",
        },
        assetTypeId: {
          type: SchemaDefinitionType.STRING,
